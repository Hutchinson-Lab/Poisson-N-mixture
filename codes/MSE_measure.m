function [MSE,A_est_order,PI] = MSE_measure(A_est,A_true)
% Input:
...
for kk=1:k
    ...
    A_est(:,kk) = A_est(:,kk)/norm(A_est(:,kk),2);
end
[A_est_order,PI] = nearest(A_est,A_true);
...
% and changes signs of the columns in the matrix, if need be, so that 
% the resultant matrix the nearest one (in sense of the Frobenius norm) 
...
for i=1:n 
     for j=1:n 
         [W(i,j) Sign(i,j)] = min([sum((Wnew(:,i)-Wold(:,j)).^2) ...
             sum((Wnew(:,i)+Wold(:,j)).^2)]); 
    end 
end 
Sign = -sign(Sign-1.5); 
        ...
        for j=1:length(sousedi) 
            if (ismember(sousedi(j),cesty(:,k:2:2*n))==0) 
                if (pridano==0) 
                    ...
                    cesty(novypocetcest,1:uroven+1) = ...
                        [cesty(i,1:uroven) sousedi(j)]; 
                end 
                pridano=pridano+1; 
            end 
        end 
